let dropdowncompleted = 0;

function popupLeftPosition(popup_width) {
	let myWidth = 0, outputWidth;
...
function popupTopPosition(target) {
	...
	let outputHeight, myHeight = 0;
	const scrollY = getScrollHeight();
...
function DropDownEffect(target, destinationTop, maxSpeed) {
	...
	let dropSpeed = 1 + Math.abs(destinationTop - currentTop) / 10;
...
function popup(popupId, effect, popup_top, delay) {
	...
	let popupStyleTop = popupTopPosition(popupElem);
	if (popup_top !== "center") {
...
function html_entity_decode(string, quote_style) {
	let hash_map = {}, symbol = '', tmp_str = '', entity = '';
...
function get_html_translation_table(table, quote_style) {
	let entities = {}, hash_map = {}, decimal = 0, symbol = '';
	const constMappingTable = {}, constMappingQuoteStyle = {};
